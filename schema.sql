...
drop policy if exists "Users manage their own grammar progress" on public.grammar_progress;
create policy "Users manage their own grammar progress" on public.grammar_progress
  for all using (auth.uid() = user_id) with check (auth.uid() = user_id);

drop policy if exists "Users manage their own reading results" on public.reading_results;
create policy "Users manage their own reading results" on public.reading_results
  for all using (auth.uid() = user_id) with check (auth.uid() = user_id);

drop policy if exists "Users manage their own app state" on public.user_state;
...
-- ============================================================
-- Telegram bot login codes (generated by @vividielts_bot /login,
-- verified by the telegram-code-login Edge Function)
-- ============================================================
...
-- No policies are defined on purpose: the anon/authenticated roles get zero
-- access, so this table is only reachable via the service-role key (the bot
-- and the Edge Function). RLS with no policies = deny all for those roles.
